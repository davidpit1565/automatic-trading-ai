...
async function reconcileExternalBuy(
  ...
  if (price === null || !(price > 0)) return; // retry next cycle rather than guessing a price
  ...
  if (!recordLiveEntryFill(store, intent, report, now)) return;
...
export async function syncManualTradesFromBroker(
  ...
  try {
    ...
    return; // network failure — reconcile next cycle, never guess
  }
  ...
  for (const instrument of CURATED_INSTRUMENTS) {
    ...
    if (diff > 0) {
      await reconcileExternalBuy(store, instrument, diff, source, telegram, now);
    } else {
