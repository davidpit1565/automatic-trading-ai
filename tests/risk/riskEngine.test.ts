...
describe("assessTrade — ignorePortfolioCapacityCaps (David's manual 'buy anyway' override, 2026-09-03)", () => {
  it('approves past the max-open-positions cap when set, but refuses without it', () => {
    const portfolio: PortfolioRiskState = {
      equity: 10_000,
      openPositions: Array.from({ length: DEFAULT_RISK_LIMITS.maxOpenPositions }, (_, i) => ({
        symbol: `COIN${i}/USD`,
    ...
    const opportunity = makeOpportunity({ symbol: 'NEW/USD' });
    expect(assessTrade(opportunity, portfolio).approved).toBe(false);
    const overridden = assessTrade(opportunity, portfolio, { ignorePortfolioCapacityCaps: true });
    expect(overridden.approved).toBe(true);
    expect(overridden.positionSize).toBeGreaterThan(0);
  });

  it('approves past the per-asset exposure cap when set, but refuses without it', () => {
    const portfolio: PortfolioRiskState = {
      equity: 10_000,
      openPositions: [{ symbol: 'BTC/USD', quantity: 20, entryPrice: 100 }], // already at the 20% cap
    };
    const opportunity = makeOpportunity({ symbol: 'BTC/USD' });
    expect(assessTrade(opportunity, portfolio).approved).toBe(false);
    expect(assessTrade(opportunity, portfolio, { ignorePortfolioCapacityCaps: true }).approved).toBe(true);
  });

  it('approves past the total-exposure cap when set, but refuses without it', () => {
    const portfolio: PortfolioRiskState = {
      equity: 10_000,
      openPositions: [
        { symbol: 'A/USD', quantity: 20, entryPrice: 100 },
        { symbol: 'B/USD', quantity: 20, entryPrice: 100 },
        { symbol: 'C/USD', quantity: 20, entryPrice: 100 },
      ], // 60% deployed — exhausts the default total-exposure cap
    };
    const opportunity = makeOpportunity({ symbol: 'D/USD' });
    expect(assessTrade(opportunity, portfolio).approved).toBe(false);
    expect(assessTrade(opportunity, portfolio, { ignorePortfolioCapacityCaps: true }).approved).toBe(true);
  });

  it('never overrides a FUNDAMENTAL refusal — invalid stop, reward:risk out of bounds, or the daily-loss circuit breaker', () => {
    const portfolio = emptyPortfolio();
    // Reward:risk far below the configured minimum — not a capacity issue.
    const badRewardRisk = makeOpportunity({ entry: 100, stopLoss: 95, takeProfit: 100.5 });
    expect(assessTrade(badRewardRisk, portfolio, { ignorePortfolioCapacityCaps: true }).approved).toBe(false);

    // Daily loss limit already exceeded.
    const dailyLossLimit = portfolio.equity * (DEFAULT_RISK_LIMITS.dailyLossLimitPct / 100);
    const halted = assessTrade(makeOpportunity(), portfolio, {
      ignorePortfolioCapacityCaps: true,
      dailyLossSoFar: dailyLossLimit + 1,
    });
    expect(halted.approved).toBe(false);
    expect(halted.reasons.some((r) => r.includes('daily loss limit'))).toBe(true);
  });

  it('still applies the single-position maxPositionPct ceiling even when portfolio-capacity caps are ignored', () => {
    // Total-exposure cap ignored, but the single-position cap (20% of
    // equity by default) must still bound the size — this only removes the
    // PORTFOLIO-wide caps, never the ceiling on any one trade.
    const portfolio: PortfolioRiskState = {
      equity: 10_000,
      openPositions: [
        { symbol: 'A/USD', quantity: 20, entryPrice: 100 },
    ...
    // A huge requested risk% (clamped to maxRiskPerTradePct anyway) doesn't
    // matter here — what's under test is the maxPositionPct cap surviving.
    const opportunity = makeOpportunity({ symbol: 'D/USD' });
    const overridden = assessTrade(opportunity, portfolio, { ignorePortfolioCapacityCaps: true });
    expect(overridden.approved).toBe(true);
    expect(overridden.positionValue).toBeLessThanOrEqual(portfolio.equity * (DEFAULT_RISK_LIMITS.maxPositionPct / 100) + 1e-9);
  });
});
