...
**Merge note (2026-09-06):** three items above turned out, once merged
against the latest `main`, to be exact duplicates of fixes `PR #203`
(Markets/coin-detail, round 1) had already shipped for the identical
elements — this audit's own snapshot of `main` predated that PR's merge:
Market Scan's `.scan-row` `role="button"`/`tabIndex` (item, `marketScanView.
ts`), the Trade tab's Buy/Sell `aria-pressed` wiring, and the base
`role="tab"`/`aria-selected` markup on the coin-detail view-tabs (both in
`marketsView.ts`). Dropped the duplicate re-implementations during merge,
keeping main's originals; kept this pass's genuinely-additive `tabindex="-1"`
roving-tabindex markup layered on top of the view-tabs (main's own version
