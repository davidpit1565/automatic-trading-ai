 ...
 * only formal "champion" this project has ever had, until now expressed
 * only as a test assertion ("a like-for-like baseline must always be
 * running") and a narrative comment in `autopilotRunner.mts`, never a named
 * constant. Naming it here is what lets `compareToChampion` (below) find
 * it without a caller having to know or guess which key that is.
 */
export const CHAMPION_KEY = 'live-mirror';

/**
 * Compares a challenger's standing against the champion's — the missing
 * "Champion vs Challenger" step an audit found (2026-09-19): promotion
 * today is a human eyeballing raw numbers with no structured comparison at
 * all. Deliberately does NOT collapse the comparison into one "wins"
 * verdict (this project's own rule: don't judge by profit factor alone,
 * and a real decision trades metrics off against each other) — it reports
 * which named metrics each side leads on, and leaves the judgment to a
 * human. Returns `comparable: false` when EITHER side has too short a
 * record to mean anything (`SHADOW_MEANINGFUL_TRADES`), including the
 * champion itself — a brand-new champion is not yet a reliable baseline.
 *
 ...
 * needed validation against real history and David's explicit approval —
...
export function compareToChampion(champion: ShadowStanding, challenger: ShadowStanding): ChallengerComparison {
